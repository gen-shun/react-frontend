...
//  constructor(private baseUrl: string = "http://20.48.15.81:8080") {}
  ...
  getCustomerProfiles(): Promise<CustomerProfile[]> {
    ...
    return axios
      .get<CustomerProfileDTO[]>(url, {
        headers: { Accept: "application/json" },
      })
      .then((response) => {
        return response.data.map(
          (dto) =>
            new CustomerProfile(dto.id, dto.firstName, dto.lastName, dto.email)
        );
      });
  }
